...
const resolveLogContext = async function (bot) {
  if (!bot || typeof bot._getLogContext !== 'function') {return '';}

  const context = await bot._getLogContext();
...
exports.sendLogMessage = async function (bot, logMessage) {
  ...
  let log = `${resolveLogPrefix(bot)}: ${logMessage}${await resolveLogContext(bot)}
env: ${env}`;
...
exports.sendErrorMessage = async function (bot, errorMessage) {
  ...
  const env = resolveLogEnv();
  let log = `${resolveLogPrefix(bot)}: ${errorMessage}${await resolveLogContext(bot)}
env: ${env}`;
