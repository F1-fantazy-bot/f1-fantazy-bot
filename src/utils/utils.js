...
exports.validateJsonData = async function (
  ...
  if (!jsonData.Drivers || jsonData.Drivers.length !== 20) {
    ...
    await bot
      .sendMessage(
        chatId,
        t('Invalid JSON data. Please ensure it contains 20 drivers under "Drivers" property.')
      )
  ...
  if (!jsonData.Constructors || jsonData.Constructors.length !== 10) {
    ...
    await bot
      .sendMessage(
        chatId,
        t('Invalid JSON data. Please ensure it contains 10 constructors under "Constructors" property.')
      )
      .catch((err) => console.error('Error sending JSON error message:', err));
  ...
  if (
    ...
    await bot
      .sendMessage(
        chatId,
        t('Invalid JSON data. Please ensure it contains the required properties under "CurrentTeam" property.')
      )
      .catch((err) => console.error('Error sending JSON error message:', err));
...
exports.triggerScraping = async function (bot, chatId) {
  ...
  if (!url) {
    await bot.sendMessage(
      chatId,
      t('Error: Scraping trigger URL is not configured.')
    );
