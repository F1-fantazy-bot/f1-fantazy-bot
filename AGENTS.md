...
PR #241 and Phase 11's confirmed admin identity/access writes are in progress.
...
| `src/agent/readTools/adminReadTools.js` | Centrally admin-guarded version, billing, directory, and BotFather reads. The two directories optionally return bounded canonical clickable targets for Phase 11 admin writes; all tools use `wrapToolExecute`, keep raw backend errors out of results, and add the authenticated user's saved UI language. |
