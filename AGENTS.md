...
- `leagues/{leagueCode}/league-standings.json` — header + `teams: [{ teamName, userName, teamNo, position, totalScore, raceScores, raceBudgets, chipsUsed }]`. Used by `/leaderboard` and `/league_graphs`.
- `leagues/{leagueCode}/teams-data.json` — header + `teams: [{ teamName, userName, teamNo, position, budget, transfersRemaining, drivers, constructors }]` where each roster entry is `{ id, name, price, isCaptain, isMegaCaptain, isFinal }`. Used by `/teams_tracker` to load/manage followed team rosters from the league directly into the bot's cache.
- `leagues/{leagueCode}/locked/matchday_{N}.json` — one blob per locked matchday written by the upstream `MODE=locked` scrape (fires ~1 minute after each session start: qualifying, race, and on sprint weekends the sprint race). Same per-team shape as `teams-data.json` (including `teamNo`) plus a top-level `mode: 'locked'` discriminator and per-team `chipsUsed: [{ name, gameDayId }]`. Used by `/league_changes` and `/live_score`.

`teamNo` mirrors the F1 Fantasy API's `team_no` field (1/2/3) — F1 Fantasy lets each account run up to 3 teams. Combined with `userName` it forms a stable cross-league identifier `{userName}_{teamNo}` that the bot uses to highlight the user's active team in every league it appears in (see `src/utils/activeTeamIdentity.js`). Blobs scraped before `teamNo` was added simply omit the field; the bot degrades to same-league-only highlighting until the next upstream scrape rewrites them.
...
The leaderboard is rendered compactly (position, team name, total score) with a header showing league name, member count, and fetch time. Teams from the blob are already sorted by `position`. The user's active `selectedTeam` row is wrapped in `<b>…</b>` — and the same bolding fires in every other followed league where the same F1 Fantasy team appears, via the cross-league `{userName}_{teamNo}` match described under "Active-team highlighting" below.

### Active-team highlighting (cross-league)

`/league_graphs` (all three sub-charts) and `/leaderboard` highlight the user's active `selectedTeam` in every followed league it appears in — not just the league it was originally picked from. The match rule, centralised in `src/utils/activeTeamIdentity.js`, is:

- **Same league** → exact `teamId` match (`{leagueCode}_{sanitizedTeamName}`), preserving the original behavior.
- **Cross league** → match by `{userName}_{teamNo}` (the F1 Fantasy account login + which of that account's up-to-3 teams). This is rename-proof and league-agnostic.

The async resolver `resolveActiveTeamFantasyId(chatId)` reads the active `selectedTeam`, looks up the league blob it came from, and returns `{ teamId, fantasyId }`. Each call site (3 graph senders + `sendLeaderboard`) calls it once before rendering and passes the result through to `isHighlightedTeam(team, leagueCode, highlight)`. Screenshot teams (`T1`/`T2`/`T3`) return `null` from the resolver — they have no F1 Fantasy account anchor, so cross-league highlighting is skipped (same-league teamId match still works as before via the `selectedTeam` fallback in `formatLeaderboard`). Blobs without `teamNo` (legacy data) silently fall back to same-league-only highlighting until the upstream scrape refreshes them.
