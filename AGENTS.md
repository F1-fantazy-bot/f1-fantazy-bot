...
- **Caching:** `src/cache.js` holds in-memory data for drivers, constructors, current team info, simulations, next race info, weather forecasts, a cached remaining-race count, canonical prices, and a unified `userCache`. Team-related caches (`currentTeamCache`, `bestTeamsCache`, `selectedChipCache`) are **nested by team ID** — see the [Multi-Team System](#multi-team-system) section below. `src/cacheInitializer.js` populates those caches on startup — most data comes from Azure Blob Storage (with team-aware blob naming), while `userCache` is populated from the `UserRegistry` Azure Table via a single `listAllUsers()` call. Driver/constructor projections still come from `f1-fantasy-data.json`, but prices come from the root `prices.json` blob and are overlaid through `getDriversForChat(chatId)` / `getConstructorsForChat(chatId)` for calculations. Imported chat-specific driver/constructor data remains raw in cache and supplies fallback prices/projections when the canonical price blob is missing or incomplete. Each entry in `userCache` is keyed by `chatId` and holds `{ lang, nickname, chatName, selectedTeam, ... }`.
...
- The frontend (`web/`) is deployed to an Azure Static Web App (`f1-fantazy-agent-web`, Free SKU). Push to `main` → `production` environment; PR → per-PR preview environment (the SWA built-in feature, no separate resource per PR). The preview environment's `VITE_AGENT_API_URL` points at the agent Function App's `test` slot so PR builds exercise the end-to-end stack against the slot version of the backend.
