...
9. `/live_score` (admin-only, `src/commandsHandler/liveScoreHandler.js`) scores the user's **real locked team** by combining the latest `live-score-data.json` blob with a roster resolved by `resolveLiveScoreTeam(chatId, teamId)`. Resolution priority: (a) for league teams (`{leagueCode}_{slug}`), the latest `leagues/{code}/locked/matchday_{N}.json` snapshot — captain/mega-captain are read from per-driver `isCaptain` / `isMegaCaptain` flags, names are mapped to bot codes via `mapNameToCode`; (b) fallback to `currentTeamCache[chatId][teamId]` (screenshot teams or league teams without a locked snapshot yet). The Boost x2 / Extra Boost x3 multipliers fire automatically off `boostDriver` / `extraBoostDriver`. The output message includes a `Source: locked snapshot · md N` (or `current team`) tag so it's obvious which roster is being scored.
