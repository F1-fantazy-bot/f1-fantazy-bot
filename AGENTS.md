...
`data/announcements.json` is a committed array of release-announcement entries (newest first). The `release-announcement` skill **writes** to it (after the admin picks Standard/WOW); `src/announcementsService.js` **reads** it, `src/cores/announcementsCore.js` builds the public `ok`/`empty` result, and both `/whats_new` plus the agent's `get_whats_new()` consume that result. Each entry has shape `{ id, createdAt, version: 'standard'|'wow', sinceRef, headCommit, text }` where `text` is the Hebrew Markdown body **without** the `### 📋`/`### 🔥` title line, fence wrappers, or backticks around `/commands`. The Telegram handler sends `text` with `parse_mode: 'Markdown'` (with plain-text fallback on parse errors) and escapes underscores inside `/command` tokens at send time so Telegram auto-links them instead of consuming the underscore as an italic marker. The agent card renders the same source text safely, with localized English/Hebrew chrome and RTL. Missing or malformed file → both surfaces show a localized "no announcements yet" state and the skill treats it as `[]`.
...
- `leagues/{leagueCode}/league-standings.json` — header + `teams: [{ teamName, userName, position, totalScore, raceScores, raceBudgets, chipsUsed }]`. Used by `/leaderboard`, `/league_graphs`, and `/race_summary`.
...
manual triggers merged in PR #244, and Phase 14 parity closure merged in PR #247.
...
- `get_league_changes` — planning-to-locked comparison for every team in one
  followed league. Omitting `leagueCode` returns the authenticated user's
  followed leagues as clickable cards; selecting one continues with its
  canonical code. Structured statuses cover no followed leagues, unauthorized
...
- `get_race_summary` — saved-language post-race recap for one followed league.
  Omitting `leagueCode` returns authenticated clickable league cards, and the
  canonical code is authorized before league storage reads. The agent shares
  the Telegram core, exclusions, roster fallback, prompt, and model. Nested
  generation has explicit token/error telemetry and hard output limits;
  missing, empty, generation, and outer tool failures remain localized and do
  not expose raw provider or storage details.
- `get_whats_new` — the latest committed release announcement, from the same
  `announcementsService` record as Telegram `/whats_new`. It has no arguments,
...
`get_action_choices({ action: 'get_best_teams', choice: 'team', context })`
and waits for a canonical team-card click — then runs `get_best_teams` ONCE
with the preserved filters/ranking and chosen team. This keeps the chat to a single rich render per question
...
3. **Yes** changes the matching chat-owned row to `approved` using its
   ...
   the confirmation turn. CopilotKit renders only `user`/`assistant`
   messages, and `chatHistoryStore` drops developer rows, so the nonce is
   neither shown nor persisted. Do not mix legacy `appendMessage` with a
...
- `web/src/components/WriteConfirmCard.tsx` — server decision first,
  then hidden developer `agent.addMessage` + coordinated
...
**Clickable clarification is mandatory for finite choices.** Use an existing
guided picker or `get_action_choices` with the pending action and preserved
arguments. Do not replace cards with prose lists of team IDs, league codes,
chips, languages, ranking presets, or driver/constructor filters. Account
options must come from the authenticated user's data. `ActionChoicesCard`
continues through the same agent, shares `agentRunLock`, rolls back failed
injected messages, and never approves writes. Read selection must not change
the active team. Shared write rendering also turns validated
`availableTeams`/`availablePresets`/`availableChips` failures into cards.
The complete routing audit is in
[`docs/agent-clickable-choice-audit.md`](docs/agent-clickable-choice-audit.md).
...
| `src/services/adminAccessService.js` | Shared admin nickname and web-access mutation service. Telegram pending replies and agent tools use its normalized exact-email handling, fresh user/allowlist checks, no-op detection, commit-time target fingerprints, persistence, and nickname cache publication. |
| `src/services/adminMessagingService.js` | Shared administrator delivery service. Telegram and agent writes use its fresh target/audience reads, recipient-localized prefixes, text chunking, commit-time broadcast audience fingerprint, bounded safe outcomes, and correlation-audit seam. Telegram photo delivery remains supported only through its existing reply flow. |
| `src/services/manualTriggerLeaseService.js` | Durable Azure Table job-scoped lease for manual Logic App requests. It assigns safe run references, deduplicates active/recent job dispatches across Function workers, handles expiry through ETag replacement, and preserves ambiguous post-dispatch leases to prevent duplicate runs. |
...
| `src/agent/readTools/adminReadTools.js` | Centrally admin-guarded version, billing, directory, and BotFather reads. The directories optionally return bounded canonical clickable targets for Phase 11 identity/access or Phase 12 direct-message writes; all tools use `wrapToolExecute`, keep raw backend errors out of results, and add the authenticated user's saved UI language. |
| `src/agent/writeTools/adminAccessTools.js` | Centrally guarded confirmed `set_user_nickname`, `allow_web_user`, and `revoke_web_user` tools. They stage only canonical inspected targets, revalidate at commit, return safe no-op/stale envelopes, and inherit audit logging from `defineAdminWriteTool`. |
| `src/agent/writeTools/adminMessagingTools.js` | Centrally guarded confirmed `send_user_message` and `broadcast_message` tools. They accept only text, stage canonical/direct recipients or a broadcast audience fingerprint, recheck at commit, return safe delivery counts, and add actor/target correlation logs. |
| `src/agent/writeTools/adminManualTriggerTools.js` | Centrally guarded confirmed tools for the five manual Logic App jobs. They provide impact-specific saved-language previews, invoke `manualTriggerService` through durable job leases, return only safe run references, and audit actor/job/outcome correlation. |
...
| `web/src/components/AdminReadCards.tsx` | Admin-only rich renders for version, bounded billing, bot/web user directories, and BotFather setup. Directory selection modes provide localized canonical nickname/allow/revoke targets with the shared agent-run lock and rollback. All cards localize English/Hebrew, set RTL, format dates in `Asia/Jerusalem`, use accessible tables, and show safe forbidden/unavailable/empty/capped states. |
...
| `web/src/components/LeagueTeamsAction.tsx` | `list_league_teams` rich render — current league roster; `follow_team` selection mode marks already-tracked teams and stages canonical add proposals. Successful follows remain marked in the card and release the selection lock so another team can be followed from the same card. |
