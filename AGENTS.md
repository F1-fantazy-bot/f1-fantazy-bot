...
7. `/league_graph` (admin-only) renders a line chart of cumulative score per race for every team in a followed league. Same 0/1/N league-selection flow as `/leaderboard` (callback type `LEAGUE_GRAPH_CALLBACK_TYPE`). Chart rendering is delegated to [`quickchart-js`](https://quickchart.io) — the handler builds a Chart.js config via `buildChartConfig(leagueData, { roundToFlag })`, calls `chart.getShortUrl()`, and sends the URL via `bot.sendPhoto` (Telegram fetches the PNG itself, no native `canvas` dep). Chip usage is drawn as an emoji + chip-name label on the specific data point using the `chartjs-plugin-datalabels` plugin (bundled with QuickChart). X-axis labels are `{flag} R{N}` — `matchday_N` is mapped to round `N` in the current Jolpica/Ergast season schedule (`fetchCurrentSeasonRaces`) and the country is converted to a flag emoji via `src/utils/countryFlags.js`; falls back to `R{N}` if the mapping can't be resolved. Chip → emoji mapping lives in `src/utils/chipEmojis.js`.
